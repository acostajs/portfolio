...
async def test_visitor_tracking_exclusions(client):
    ...
    await client.get(
        "/api/v1/admin/analytics/summary", headers={"x-admin-token": "testpass"}
    )
    ...
    with Session(database.engine) as session:
        ...
        stmt = select(VisitorLog).where(
            col(VisitorLog.path).in_(
                ["/api/v1/admin/analytics/summary", "/styles/main.css", "/docs"]
            )
        )
...
async def test_analytics_visitors_pagination(client, db_session):
    ...
    for i in range(10):
        db_session.add(
            VisitorLog(path=f"/test-{i}", method="GET", ip_hash="hash", locale="en")
        )
    db_session.commit()

    response = await client.get(
        "/api/v1/admin/analytics/visitors", headers={"x-admin-token": "testpass"}
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) >= 10
