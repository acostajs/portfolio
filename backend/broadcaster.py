...
class Broadcaster:
    ...
    def subscribe(self, session_id: str) -> asyncio.Queue:
        ...
        logger.debug(
            f"Subscribed queue to session {session_id}. Active: {len(self.subscribers[session_id])}"
        )
        return queue
    ...
    async def broadcast(self, session_id: str, event_type: str, data: Any):
        ...
        if session_id in self.subscribers:
            logger.debug(
                f"Broadcasting {event_type} to {len(self.subscribers[session_id])} subscribers for {session_id}"
            )
